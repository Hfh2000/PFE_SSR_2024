...
echo "Compilation et empaquetage du Smart Contract Public "
cd ../asset-handle-public/chaincode-javascript
npm install
cd ../../test-network
...
export FABRIC_CFG_PATH=$PWD/../config/
peer version
peer lifecycle chaincode package public.tar.gz --path ../asset-handle-public/chaincode-javascript/ --lang node --label public_1.0

echo "Installation du chaincode sur Org1 et Org 2"
export CORE_PEER_TLS_ENABLED=true
export CORE_PEER_LOCALMSPID="Org1MSP"
export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/org1.example.com/users/Admin@org1.example.com/msp
export CORE_PEER_ADDRESS=localhost:7051
peer lifecycle chaincode install public.tar.gz
...
echo "Récupération de l'identifiant du package de chaincode"
export CC_PACKAGE_ID=$(peer lifecycle chaincode queryinstalled | grep "public_1.0:" | awk '{print $3}' | sed 's/,//g')

echo "Approbation du chaincode par Org2"
peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --channelID public-channel --name public --version 1.0 --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem"
...
echo "Vérification de l'état d'approbation"
peer lifecycle chaincode checkcommitreadiness --channelID public-channel --name public --version 1.0 --sequence 1 --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" --output json

echo "Engagement du chaincode sur le canal"
peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --channelID public-channel --name public --version 1.0 --sequence 1 --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" --peerAddresses localhost:7051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt" --peerAddresses localhost:9051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt"

echo "Vérification de l'engagement du chaincode"
peer lifecycle chaincode querycommitted --channelID public-channel --name public --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem"
...
echo "Vérification des actifs enregistrés"
sleep 3
peer chaincode query -C public-channel -n public -c '{"Args":["GetAllAssets"]}'
